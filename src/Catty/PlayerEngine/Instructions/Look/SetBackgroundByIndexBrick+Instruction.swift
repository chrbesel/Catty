...
@objc extension SetBackgroundByIndexBrick: CBInstructionProtocol {

    @nonobjc func instruction() -> CBInstruction {
        .action { context in SKAction.run(self.actionBlock(formulaInterpreter: context.formulaInterpreter,
                                                           imageCache: RuntimeImageCache.shared())) }

    }
    @objc func actionBlock(formulaInterpreter: FormulaInterpreterProtocol,
                           imageCache: RuntimeImageCache) -> () -> Void {
