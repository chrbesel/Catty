...
@objc(StopScriptBrick)
@objcMembers class StopScriptBrick: Brick, BrickProtocol, BrickStaticChoiceProtocol {

    static let defaultSelection = StopScriptStyle.thisScript

    var selection: StopScriptStyle
    ...
    func setChoice(_ message: String, forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) {
        self.selection = StopScriptStyle.from(localizedString: message) ?? type(of: self).defaultSelection
    }

    func possibleChoices(forLineNumber lineNumber: Int, andParameterNumber paramNumber: Int) -> [String] {
        StopScriptStyle.allCases.map { $0.localizedString() }
    }
}
